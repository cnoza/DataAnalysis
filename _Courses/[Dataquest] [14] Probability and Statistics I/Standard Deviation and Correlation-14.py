...
point_variance = sum([(each - pts_mean) ** 2 for each in nba_stats['pts']])/len(nba_stats['pts'])
...
def std_dev(series):
    ...
    series_variance = sum([(each - series_mean) ** 2 for each in series])/len(series)
    return series_variance ** .5
...
import numpy as np
wl_mean = np.mean(wing_lengths)
wl_std = np.std(wing_lengths)
wl_dist = np.array([(i - wl_mean)/wl_std for i in wing_lengths])

within_one_percentage = float(sum(abs(wl_dist) < 1)/len(wing_lengths))
within_two_percentage = float(sum(abs(wl_dist) < 2)/len(wing_lengths))
within_three_percentage = float(sum(abs(wl_dist) < 3)/len(wing_lengths))
...
def covar(x,y):
    mean_x = x.mean()
    mean_y = y.mean()
    return sum([(x_i - mean_x)*(y[i]-mean_y) for i, x_i in enumerate(x)])/len(x)

cov_stl_pf = covar(nba_stats['stl'],nba_stats['pf'])
cov_fta_pts = covar(nba_stats['fta'],nba_stats['pts'])
