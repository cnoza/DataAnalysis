...
degrees_zero = [float(each) + 459.67 for each in fahrenheit_degrees]
...
average_smoking = np.mean([ scale[each] for each in survey_responses ])
...
# First way:
male_savings = np.mean([savings[i] for i,v in enumerate(gender) if gender[i] == 'male'])
female_savings = np.mean([savings[i] for i,v in enumerate(gender) if gender[i] == 'female'])

# Second way:
import pandas as pd
data = pd.DataFrame({'gender': gender, 'savings': savings})
male_savings = data[data['gender']=='male'].mean()
female_savings = data[data['gender']=='female'].mean()
...
plt.axvline(new_titanic_survival['age'].median(), color='g')
