...
intercept_density = calc_intercept(wine_quality['density'], wine_quality['quality'], slope_density)
...
residuals = np.asarray([(value - predicted_y[i]) ** 2 for i,value in enumerate(wine_quality['quality'])])
rss = sum(residuals)

std_e = (rss/(len(residuals)-2)) ** .5

wq_diff = np.asarray([(value - predicted_y[i]) for i,value in enumerate(wine_quality['quality'])])

within_one = float(sum(abs(wq_diff)/std_e <= 1 )) / len(wq_diff)
within_two = float(sum(abs(wq_diff)/std_e <= 2 )) / len(wq_diff)
within_three = float(sum(abs(wq_diff)/std_e <= 3 )) / len(wq_diff)